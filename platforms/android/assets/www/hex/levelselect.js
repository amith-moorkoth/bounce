...
LevelSelect.prototype = {
	...
	preload: function() {
		this.game.load.spritesheet('levelselecticons', 'levelselecticons.png', 66, 66);
	...
	create: function() {
		this.game.stage.backgroundColor = 0x33383c;
        var str = window.localStorage.getItem('myvelselector');
        if (str==null ) {
            str=0;
        }str-=1;str+=2;

		//this.game.add.bitmapText(256, 24, 'font72', 'Level '+str, 28);
	...
	createLevelIcons: function() {
        ...
        var str = window.localStorage.getItem('myvelselector');
        if (str==null ) {
            window.localStorage.setItem('myvelselector', 0);
        } else{
            myvelselector = str;
        };
		...
		for (var y=0; y < 3; y++) {
			for (var x=0; x < 4; x++) {
				// next level
				levelnr_inc = levelnr_inc + 1;
				levelnr =levelnr_inc+(12*myvelselector);
				...
				var xpos = ((CANVAS_WIDTH*3)/60) + (x*78);
				var ypos = ((CANVAS_HEIGHT*3)/14) + (y*(CANVAS_HEIGHT/5));
	...
	createLevelIcon: function(xpos, ypos, levelnr, isLocked, stars) {
		...
		// add stars, if needed
		if (isLocked == false) {
			var txt = this.game.add.bitmapText(24, 16, 'font72', ''+levelnr, 12);
	...
	onLevelSelected: function(levelnr) {
        ...
        window.localStorage.setItem('curlevel', levelnr);
